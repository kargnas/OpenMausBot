...
export async function connectionStatus(cfg, slugs) {
    ...
    const [res, accounts] = await Promise.all([
        ...
        userId
            ? fetch(`${apiBase()}/connected_accounts?${new URLSearchParams({ limit: "50", user_ids: userId })}`, { headers: projectHeaders(cfg.composio.apiKey), signal: AbortSignal.timeout(15_000) })
    ...
    for (const account of accounts) {
        ...
        if (!slug || !slugs.some((candidate) => candidate.toLowerCase() === slug))
            continue;
