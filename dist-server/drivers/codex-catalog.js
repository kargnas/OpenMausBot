...
import { mergeLocalInject } from "./local-inject.js";
...
export function encodeCodexSelection(provider, model) {
    return `${provider}${SEP}${model}`;
}
export function decodeCodexSelection(id) {
    if (!id)
...
async function probeProviderModels(provider, env, fetchImpl) {
    if (!provider.baseUrl)
        return [];
...
export async function readCodexModelCatalog(env = process.env, fetchImpl = fetch) {
    ...
    const remember = (provider, model) => {
        ...
        if (provider === OFFICIAL_CODEX_PROVIDER &&
    ...
    for (const [encoded, _label] of named) {
        ...
        if (decoded.model && decoded.modelProvider)
            remember(decoded.modelProvider, decoded.model);
    }
    const live = await Promise.all([...known.values()].map(async (provider) => {
        const ids = await probeProviderModels(provider, env, fetchImpl);
        return ids.map((model) => ({ provider: provider.id, model }));
    }));
    for (const row of live.flat())
        remember(row.provider, row.model);
    ...
    return mergeLocalInject({
        default: {
            model: configured && seen.has(configured) ? configured : STATIC_CODEX_MODELS.default.model,
        },
        options,
    }, env, fetchImpl);
}
