...
export function readGrokModelCatalog(env = process.env) {
    ...
    for (const line of text.split(/\r?\n/)) {
        ...
        if (current && key === "name" && value)
            current.name = value;
        if (!current && inModels && key === "default")
...
function suggestGrokSlug(host, model, taken) {
...
export function ensureGrokInjectSlug(modelId, env = process.env) {
    ...
    const taken = new Set(STATIC_GROK_MODELS.options.map((option) => option.id));
    let current = null;
    const flush = () => {
        if (!current)
            return;
...
const support = {
    ...
    displayName: "Grok",
    // File-based slugs (~/.grok/config.toml + live local-host injects) back the
    // picker when initialize's _meta probe can't run; core's probeCatalog takes
    // precedence for a live CLI.
