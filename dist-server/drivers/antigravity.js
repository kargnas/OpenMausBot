...
export const AntigravityDriver = {
    ...
    async create(input) {
        ...
        const catalog = () => new Promise((resolve, reject) => {
            execCli(config.cli, ["models", "--output-format", "json"], { timeout: 20_000, env: { ...process.env, ...input.environment, PATH: augmentedPath() } }, (error, stdout) => {
                if (error)
                    return reject(error);
                try {
                    ...
                    const options = (Array.isArray(listed) ? listed : [])
                        ...
                        const efforts = (Array.isArray(listedEfforts) ? listedEfforts : [])
                            .map((effort) => typeof effort === "string" ? effort : effort?.effort ?? effort?.reasoningEffort ?? effort?.id)
                    ...
                    if (!options.length)
                        throw new Error("Antigravity models returned no models");
                    const requested = typeof payload?.defaultModel === "string"
                        ? payload.defaultModel
                        : listed.find((model) => model?.default === true)?.id;
                    const model = requested && options.some((option) => option.id === requested)
                        ? requested
                        : options[0].id;
